...
// которым идут: до трёх закрывающих знаков (кавычка, точка, запятая и т.п. —
// без них модель, желая закрыть кавычку, вынуждена продолжать иероглифами и
// городит мусор), необязательный пробел, «(», от 1 до 80 знаков перевода и
// «)». Перевод — только кириллица, цифры, пробел и знаки препинания: латиница
// в скобках запрещена, иначе модель кладёт туда пиньинь вместо перевода. Пока
// кусок иероглифов не закрыт переводом, закончить ответ нельзя. llama.cpp сначала выбирает токен обычным
...
//    будет.
//  - Латинское слово в переводе («ДНК» пройдёт, «DNA» — нет) невозможно; это
//    цена запрета пиньиня.
...
Java_com_dark_gguf_1lib_GGUFNativeLib_nativeGenerateStreamMultiTurn(
    ...
    /*
     * Список токенов, с которым сверяется начало СЛЕДУЮЩЕГО запроса
     * (`find_common_prefix` выше). Здесь в него кладётся запрос, а ниже, в
     * цикле, дописывается каждый сгенерированный токен.
     *
     * ЗАЧЕМ ДОПИСЫВАТЬ. Без этого список равен одному запросу, а в кэше
     * лежит запрос ПЛЮС ответ. На следующем ходе совпадение обрывается там,
     * где кончился прошлый запрос, `llama_memory_seq_rm` выбрасывает всё
     * дальше — то есть прошлый ответ, — и он считается заново.
     *
     * ЗАМЕРЕНО 28.08.2026 на устройстве, ДО правки: ход с ответом в 406
     * токенов — до первого токена 2.0 с; следующий короткий ход, запрос 783
     * токена — до первого токена 38.1 с. Разница ровно в размер прошлого
     * ответа при ~11 токенах в секунду обсчёта. Это секундомер, а не
     * рассуждение по коду.
     *
     * ЧЕГО ЖДАТЬ ПОСЛЕ. Тот же второй ход должен стоить две-три секунды
     * вместо сорока. Первый прогон после правки не показателен: кэш пуст.
     *
     * ГДЕ ТЕРЯЕТСЯ НЕМНОГО. Если генерацию оборвала стоп-строка, её текст из
     * ответа вырезается, а токены в кэш уже легли. Список и текст разойдутся
     * на длину стоп-строки, и следующий запрос пересчитает этот хвостик. Это
     * единицы токенов и правильное поведение: сверка честно найдёт
     * расхождение и пойдёт от него. Обрезать список под текст нельзя —
     * границы токенов по знакам не восстанавливаются.
     *
     * ДВЕ ДРУГИЕ ВЕТКИ (одноходовая ~1533 и картиночная) НАМЕРЕННО НЕ
     * ТРОНУТЫ: оба наших пути заходят сюда, те не используются, и править
     * их значило бы менять непроверяемое. Асимметрия названа здесь, чтобы
     * следующий читатель не счёл её недосмотром.
     */
    g_state.prev_prompt_tokens = tokens;
    bool track_prev_tokens = true;
    ...
    while (n_generated < maxTokens && !g_state.cancel_flag.load()) {
        ...
        if (g_state.n_past >= (int)llama_n_ctx(g_state.ctx) - 1) {
            ...
            // Сдвиг выбросил середину и очистил prev_prompt_tokens: позиции
            // в кэше больше не отвечают списку, дописывать в него нечего.
            // Следующий запрос честно пересчитается целиком.
            track_prev_tokens = false;
        }
        ...
        common_batch_add(sb, id, g_state.n_past, {0}, true);
        if (llama_decode(g_state.ctx, sb) != 0) break;
        // Сгенерированный токен дописывается в список сразу после того, как
        // лёг в кэш. Пояснение — там же, где список присваивается.
        if (track_prev_tokens) g_state.prev_prompt_tokens.push_back(id);
        g_state.n_past++;
        n_generated++;
        auto ts4 = std::chrono::high_resolution_clock::now();
        g_state.last_decode_us += std::chrono::duration_cast<std::chrono::microseconds>(ts4 - ts3).count();
