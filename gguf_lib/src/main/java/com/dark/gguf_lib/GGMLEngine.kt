...
class GGMLEngine {
     ...
     * [updateSamplerParams].
     *
     * ЗЕРНО ЗАДАЁТ ДВА РАЗНЫХ РЕЖИМА, и оба нужны.
     *
     * Отрицательное значение — «выбирает система»: зерно берётся заново при
     * каждом вызове, повторить прогон дословно нельзя. Это то, что нужно
     * разговору.
     *
     * Неотрицательное — «ровно это число»: сэмплер стартует из одного и того
     * же состояния, и при том же запросе получится тот же ответ. Это
     * единственный способ поставить повторяемый опыт на модели — сравнить две
     * настройки, не гадая, что изменилось от случайности.
     *
     * Почему сказано так подробно: до 02.09.2026 нативная часть разворачивала
     * отрицательное зерно в константу, то есть молча включала второй режим
     * вместо первого. Внешне это выглядело поломкой модели — близкие вопросы
     * получали дословно одинаковые ответы. Обещание давалось здесь, а
     * нарушалось там, и разошлись они беззвучно.
     *
     * Чего эта настройка не даёт: даже при заданном числом зерне ответ зависит
     * ещё и от того, сколько токенов запроса переиспользовано из уже
     * обсчитанного начала. Одно зерно воспроизводимости не гарантирует.
     *
     * @param seed отрицательное — случайное зерно на каждый вызов;
     *        неотрицательное — фиксированное, для повторяемых прогонов.
     */
    fun setSampling(
