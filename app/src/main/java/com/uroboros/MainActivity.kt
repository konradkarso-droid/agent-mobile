        ...
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "В разговоре ${journal.turnCount} ходов, занято " +
                    ...
                    "откат. Сказанное в разговоре в память уже легло само; кнопка сохранения " +
                    "нужна только для текста, который репликой не был."
            )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonGenerate.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                // Сказанное ложится в память само. Провенанс явный и здесь так
                // же верен, как на кнопке сохранения: это ввод человека.
                //
                // ПОСЛЕ ОТБОРА, И ПОРЯДОК ЗДЕСЬ НЕСУЩИЙ. Сохрани мы раньше —
                // свежая запись нашлась бы отбором на этом же ходе, и одно
                // утверждение уехало бы в модель дважды: репликой человека и
                // цитатой записи рядом с ней. Два вхождения одного утверждения в
                // разных обрамлениях читаются как два свидетельства, а не как
                // одно сказанное дважды. Записавшись после отбора, реплика
                // уходит один раз, а запись впервые участвует со следующего
                // хода — то есть тогда, когда от неё есть польза.
                //
                // Чего здесь НЕТ и почему. Нет разбора, стоит ли это помнить:
                // ступень «нужно/не нужно» снята сознательно, разбирать должен
                // фоновый разбор памяти, а палец на кнопке был его заглушкой.
                // Нет и различения вопроса от утверждения: в память идёт речь
                // целиком, как сказана.
                //
                // Чего этот вызов НЕ покрывает: реплики, приходящие другими
                // путями — цикл задачи и вопрос, поданный агенту во время
                // цикла, — сюда не заходят и в память сами не ложатся.
