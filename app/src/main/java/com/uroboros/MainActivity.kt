...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Тот же источник, но словами, обращёнными К МОДЕЛИ.
     *
     ...
     *
     * Материал для правильной подачи был готов, его надо было просто
     * произнести словами. Это правка формулировки, а не механизма.
     */
    private fun provenancePhrase(sourceName: String): String = when (sourceName) {
        SourceKind.USER_STATED.name -> "со слов пользователя"
        SourceKind.AGENT_INFERRED.name -> "твой собственный вывод"
    ...
    /**
     * Собирает текст запроса из блока записей и вопроса.
     *
     * Принимает готовые СТРОКИ, а не список записей, намеренно: так функцию
     * видно целиком, она не тянет за собой типы из слоя памяти, и её текст
     * можно сверить глазами с тем, что уходит в модель.
     *
     * Три решения, заложенные в форму блока:
     *
     * 1. Слова "память"/"контекст" в заголовке НЕТ. Прежний заголовок "Контекст
     *    из памяти:" модель читала как тему разговора и рассуждала о своей
     *    памяти вообще, вместо того чтобы пользоваться тем, что ей дали.
     * 2. Текст записи взят в кавычки. Кавычки — это и есть механизм, которым
     *    в языке отделяют чужую речь от своей.
     * 3. Пояснение стоит ВПЛОТНУЮ к вопросу, а не в системной стене, хотя
     *    стена приходит из кэша бесплатно, а за эти строки мы платим каждый
     *    ход. Причина в собственном наблюдении (хвост 15): чем длиннее вход,
     *    тем слабее модель держит указание, стоящее далеко. Стена уедет в
     *    начало, как только появится журнал разговора, и указание оттуда
     *    перестанет работать ровно тогда, когда станет нужнее всего.
     *
     * Местоимение внутри записи НЕ переписывается ("мой" не превращается в
     * "ваш") сознательно: это правка слов пользователя. Система вправе помнить
     * неточно, но не вправе перепутать, кто что сказал. Вместо переписывания —
     * сказано словами, кому местоимение принадлежит.
     */
    private fun buildPrompt(memoryBlock: String, userText: String): String {
        if (memoryBlock.isBlank()) return userText
        return "Тебе известно следующее:\n$memoryBlock\n\n$MEMORY_USAGE_NOTE\n\nВопрос: $userText"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonGenerate.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                val prompt = buildPrompt(memoryBlock, userText)
                ...
                // поднял из кэша.
                //
                // Рамка считается ОТДЕЛЬНО от записей. Раньше в "память N зн."
                // попадал и заголовок с вопросной строкой — тогда это были
                // три десятка знаков, и разницей можно было пренебречь. С
                // появлением пояснения рамка стала весить сотни знаков, то есть
                // сравнимо с самими записями, и слитое число врало бы о том,
                // за что мы платим.
                //
                // Числа до 27.08.2026 с новыми напрямую не сравнивать: там
                // "память" включала маленькую старую рамку.
                val promptShape = if (stickers.isEmpty()) {
                    "Запрос: записей нет · вопрос ${userText.length} зн."
                } else {
                    "Запрос: записей ${stickers.size} на ${memoryBlock.length} зн. + " +
                        "рамка ${prompt.length - memoryBlock.length - userText.length} зн. · " +
    ...
    companion object {
        ...
        /**
         * Пояснение, которое идёт в запросе сразу после найденных записей.
         *
         * Вынесено в именованную константу, а не вписано в строку сборки, по
         * той же причине, по какой вынесен [ANSWER_TOKEN_LIMIT]: это величина,
         * которую мы будем подбирать по поведению модели, и её правка должна
         * быть видна одним местом, а не поиском по файлу.
         *
         * Каждое предложение здесь лечит замеченную живьём поломку и не
         * добавлено "на всякий случай" — за каждое мы платим токенами в
         * КАЖДОМ запросе:
         *  - "это записи, а не твои реплики" — против "в моей памяти нет
         *    информации" при записи, лежащей прямо в запросе;
         *  - про местоимения — против дословного пересказа с чужим "мой";
         *  - "скажи прямо" — потому что пустой результат поиска у нас
         *    нормальный исход, и честное "не знаю" лучше, чем связывание
         *    вопроса с посторонней записью.
         */
        private const val MEMORY_USAGE_NOTE =
            "Это записи, а не твои реплики. Местоимения в кавычках принадлежат " +
                "тому, кто говорил. Опирайся на записи; если ответа в них нет — " +
                "скажи прямо."
    }
}
