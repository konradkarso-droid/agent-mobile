...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Что сказала сверка на последнем ходе — о записях между собой и о записи
     * с репликой человека.
     *
     ...
     * дословно.
     *
     * ЗАЧЕМ. Панель называет состав запроса числами, а самого текста не видно
     * нигде. `ConversationJournal.Turn.userContent` хранит его только у
     * СОСТОЯВШЕГОСЯ хода: ход, на котором движок выдал ноль, в ленту не идёт,
     * и разобрать по ней нечего. Строка держится здесь, отдельно от ленты,
     * именно затем, чтобы пережить несостоявшийся ход.
     *
     * СТАВИТСЯ В МОМЕНТ СБОРКИ, а не по итогам хода, и это несущее. Между
     * сборкой реплики и концом хода стоят два выхода — переполненная лента и
     * слишком длинная реплика, — оба кончаются диалогом, и отчёта о прогоне
     * на них не бывает. Прибор, повешенный на отчёт, молчал бы ровно там, где
     * спрашивают, из чего сложилась длина.
     *
     * ТОЛЬКО ЧТЕНИЕ. Показывается та же строка, что ушла в движок, а не её
     * копия для экрана: правило дословности ленты запрещает второй текст,
     * который может разойтись с первым. Отсюда граница — причёсывать этот
     * текст для показа нельзя; понадобится причёсанный, его придётся развести
     * с этим явно.
     *
     * ЧЕГО НЕ ПОКРЫВАЕТ: путь цикла TOTE и вопрос, поданный агенту во время
     * цикла. Там реплика собирается не здесь, и этой строки для неё не
     * появится.
     *
     * Живёт один ход (стирается в [clearRunMetrics]) и перезапуска приложения
     * не переживает. Для состоявшихся ходов та же строка лежит в ленте и на
     * диске — прибор для неё отдельный и ещё не построен.
     */
    private var lastComposedContent: String? = null

    /**
     * Сколько реплик этого разговора память приняла сама, без кнопки, и
     * сколько отвергла дословным повтором.
     *
     * Два числа, а не одно: «легло» и «повтор» — разные исходы, и слитые в
     * сумму они скрыли бы самый частый способ сломаться молча. Механизм,
     * который перестал писать, даёт нули в обоих; механизм, который пишет в
     * пустоту, даёт рост повторов при неподвижном первом числе.
     *
     * Печатаются ВСЕГДА, включая ноль-ноль: строка, появляющаяся только при
     * успехе, неотличима от несработавшей автозаписи — экран выглядел бы в
     * точности как раньше.
     *
     * Живут разговор, а не ход, поэтому стираются вместе с лентой, а не в
     * [clearRunMetrics]: числа отвечают на вопрос «сколько сказанного осело за
     * этот разговор», и обнулять их после каждого ответа значило бы отвечать
     * на другой вопрос.
     */
    ...
    private suspend fun renderPendingReview(pending: List<Sticker>): CharSequence {
        ...
        // Причина одна: строка связности стоит в шапке, а считается по тем же
        // отчётам, что печатаются ниже у каждой записи. Запросов от этого не
        ...
        if (pendingHelpExpanded) {
            ...
            // Пояснение к строке связности из шапки. Стоит здесь, а не у
            // самой строки: показание видно всегда, проза к нему — под катом.
            out.append("\n\nсвязность — сколько записей ЭТОГО списка спорят ")
            out.append("с другой скрытой записью. Она отвечает на вопрос, одно ")
            out.append("это дело, разложенное на несколько записей, или разные ")
    ...
    /**
     * Строка связности: сколько записей очереди спорят с другой записью
     * очереди.
     *
     * ЗАЧЕМ. Число «На проверке: N» не отличает N разных дел от ОДНОГО дела,
     * разложенного на N записей. Близкие утверждения об одном предмете
     * попадают в память по нескольку штук, и проверка стоит на входе: первая
     * остаётся видимой, каждая следующая уходит в очередь. Человек разбирает
     * их поштучно, и что это одно дело, с экрана не видно — родня у каждой
     * записи названа своей строкой, но сложить эти строки глазами можно
     * только сличая цитаты.
     *
     * СЧИТАЕТСЯ ДАРОМ. Отчёт о спорах всё равно запрашивается на каждую
     * показанную запись, чтобы напечатать её строку «спорит». Здесь берутся
     * те же самые отчёты; нового обращения к базе строка не стоит.
     *
     * ТРИ ЗНАЧЕНИЯ, А НЕ ДВА. «Ни одна не спорит» и «проверить не удалось» —
     * разные вещи: при сбое запроса отчёт приходит пустым, и молчание такой
     * строки не значит ничего. Записи с несостоявшейся проверкой поэтому
     ...
     *    на одну запись меньше, чем сторон у дела;
     *  - связь считается по ПОКАЗАННОЙ части очереди, а противники берутся из
     *    всей горячей памяти того же тега. Запись может спорить со скрытой
     *    записью, до которой список не дошёл из-за потолка
     *    [PENDING_REVIEW_LIMIT]: она попадёт в счёт связанных, а её противник
     *    нет;
    ...
    private suspend fun acceptRecord(sticker: Sticker) {
        ...
        val pending = mediator.getPendingReview()
        binding.textResults.text = renderPendingReview(pending)
        // Вторая сторона спора ищется ПОСЛЕ приёма и по свежему состоянию.
        // Принятая запись в свой же отчёт не попадает — она исключается по id.
        //
        // Исходов три, и третий не лишний: несостоявшаяся проверка даёт пустой
        // отчёт, то есть выглядит как "спорить не с чем". Молча проглотить её
        // значило бы сказать человеку, что второй стороны нет, когда о ней
        // просто не удалось узнать.
        val report = mediator.disputesOf(sticker)
        // Число остатка называется вслух: без него приём и несработавший
        // приём выглядят одинаково.
        val tail = if (report.failed) " · вторую сторону проверить не удалось" else ""
        Toast.makeText(
            this@MainActivity,
    ...
    /**
     * Второй вопрос: что делать со стороной спора, которая осталась видимой.
     *
     * ЗАЧЕМ ОТДЕЛЬНЫЙ ДИАЛОГ, А НЕ ТРЕТЬЯ КНОПКА В ПЕРВОМ. Приём — решение про
     * принимаемую запись, скрытие противника — решение про другую. Слитые в
     * один выбор, они читаются как вердикт о том, кто прав, и тогда человек,
     * принявший запись по постороннему поводу, молча выносит приговор той, о
     * которой не думал. Два вопроса подряд, каждый про одну запись.
     *
     * ПОЧЕМУ ОПТОМ ЗДЕСЬ МОЖНО, А "ПРИНЯТЬ ВСЁ" НЕЛЬЗЯ. Скрытие обратимо:
     * запись цела, попадает в очередь и принимается назад тем же путём.
     * Приём необратим, и запрет на оптовое действие поставлен именно этим, а
     * не числом записей.
     *
     * ОТМЕНА НЕ ДЕЛАЕТ НИЧЕГО, и это направление промаха выбрано: закрыв
     * диалог, человек оставляет память ровно в том виде, в каком она была
     * секунду назад.
     *
     * ЧЕГО ЭТОТ ВОПРОС НЕ ПОКРЫВАЕТ:
     *  - пары, сошедшиеся мимо приёма. Вопрос задаётся только в момент, когда
     *    человек принимает запись; две видимые записи, переставшие ладить по
     *    другой причине, так и останутся в выдаче, и спросить о них негде;
     *  - всё, чего не берёт само правило. Оно грубое намеренно: пересказ теми
     *    же по смыслу, но другими словами не ловится вовсе, поэтому третья
     *    сторона того же спора может спокойно остаться видимой и в списке не
     *    появиться;
    ...
    /**
     * Собственно скрытие. Записи обходятся по одной, потому что запрос
     * поштучный и упасть может на любой из них.
     *
     * Исход называется числом, а не словом "готово": скрыть три и скрыть одну
     * из трёх — разные вещи, и вторую человек обязан увидеть, иначе он уйдёт
     * уверенным, что в выдаче не осталось ничего спорного.
     */
    private suspend fun hideOpponents(opponents: List<Sticker>) {
        var hidden = 0
        ...
        binding.textResults.text = renderPendingReview(pending)
        val message =
            if (hidden == opponents.size) "Убрано в очередь: $hidden · в очереди ${pending.size}"
            else "Убрано $hidden из ${opponents.size} · остальные остались в выдаче"
        ...
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "В разговоре занято ${journal.lastPromptTokens} из ${CONTEXT_SIZE} " +
                    ...
                    "В память текст тоже НЕ записан: он никуда не ушёл, значит сказанным " +
                    "не считается. Нужно его запомнить — нажмите «Сохранить».\n\n" +
    ...
    /**
     * Исход сверки — строкой для человека.
     *
     ...
     *
     * СЛОВА «ЗАПИСЕЙ» В ЗАГОЛОВКЕ ТОЖЕ НЕТ, и это не сокращение. Сверяются два
     * разных вида пар — записи между собой и запись с репликой человека, — а
     * общий заголовок обязан покрывать оба. Какой именно вид сработал, сказано
     * ниже, в самом тексте: там у каждой строки своё подлежащее.
     */
    private fun disputeNoticeLabel(result: DisputeNotice.Result): String = when (result) {
        DisputeNotice.Result.NothingToCompare -> "Сверка: сверять было нечего"
        DisputeNotice.Result.Clean -> "Сверка: расхождений нет"
        is DisputeNotice.Result.Found ->
            "Сверка: расхождений ${result.pairsFound}, названо ${result.pairsShown}\n" +
                result.text
    }

    /**
     * Строка прибора автозаписи. Печатается всегда — см. [autoSavedCount].
     *
     * «Повтор» здесь означает ровно одно: память отказала дословному совпадению
     * и новой записи не появилось. Это не ошибка и не потеря — лежащей записи
     * засчитано обращение. Число растёт, когда человек повторяет одну и ту же
    ...
    private fun renderMetricsPanel() {
        ...
        // значило бы развести вопрос и ответ на него.
        //
        // Печатается ВСЕГДА, включая случай «реплики ещё не было»: прибор,
        // появляющийся только при удаче, неотличим от неработающего. Поэтому
        // строка-заглушка говорит словами, почему показывать нечего.
        //
        // Начало участка вычисляется вычитанием, а не запоминается до вызова:
        // group дописывает разделитель между группами, поэтому длина ДО вызова
        ...
        if (composed != null) {
            ...
            metrics.setSpan(
                object : ClickableSpan() {
                    override fun onClick(widget: View) = showComposedContentDialog(composed)
                    override fun updateDrawState(ds: TextPaint) {
                        ds.color = colorRecordsLink
                        ds.isUnderlineText = false
                    }
     ...
     *
     * Длина названа в знаках, а не в токенах: пересчёт знаков в токены мы не
     * умеем, движок отдаёт настоящее число только по итогам прогона. На
    ...
    private fun composedContentLine(): String {
        ...
        return "Показать реплику целиком (${content.length} зн.)"
    }

    /**
     * Реплика пользователя, ушедшая в движок, — целиком и дословно.
     *
     * ЗАЧЕМ ОТДЕЛЬНОЕ ОКНО, А НЕ СТРОКА В ПАНЕЛИ. Реплика бывает в тысячи
     * знаков; вывалив её в шторку, мы утопили бы в ней все остальные
     * показания. Смотрят на неё по поводу, а не при каждом ответе.
     *
     * ЧЕГО ЭТО ОКНО НЕ ПОКАЗЫВАЕТ, и это сказано в нём самом, а не только
     * здесь: системной стены (задаётся отдельно при загрузке модели) и
     * прошлых ходов ленты (уходят своими сообщениями). Поэтому число знаков
     * здесь МЕНЬШЕ того, что движок называет размером запроса, и сверять их
     * между собой нельзя.
     *
     * Текст не режется и не причёсывается: смысл прибора в том, что показано
     * ровно ушедшее. Длинное окно прокручивается само.
     */
    private fun showComposedContentDialog(content: String) {
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "Ниже — реплика пользователя целиком, в том виде, в каком она ушла " +
                    ...
                    "ЭТО НЕ ВЕСЬ ЗАПРОС. Здесь нет системной стены: она задаётся " +
                    "отдельно при загрузке модели. Нет и прошлых ходов разговора: " +
                    "они уходят в движок своими сообщениями. Поэтому знаков здесь " +
                    "меньше, чем говорит счётчик размера запроса.\n\n" +
                    "— — —\n\n" +
                    content
            )
    ...
    private fun clearRunMetrics() {
        ...
        // стираются числа выше, — текст подделывается под свежий так же
        // охотно, как число.
        lastComposedContent = null
        showProgress(null)
        renderMetricsPanel()
    }
    ...
    /**
     * Строка об иероглифах в ответе; `null` — сказать нечего.
     *
     * Что делают перевод в скобках и запрет и чего они не умеют — у
     * `gloss_grammar` и `build_script_ban` в `gguf_lib.cpp`; здесь только
     * перевод состояния в слова.
     *
     * Когда всё в порядке и иероглифов не было, строка молчит: переведённые
     * места и так видны в ответе. Кричит она о поломке перевода — её видно
     * только в ответе с иероглифом, а он бывает редко, и до тех пор «перевода
     * не было, потому что нечего переводить» и «перевод сломан» на экране
     * неотличимы.
     */
    private fun hieroglyphLine(b: GGMLEngine.DecodeBreakdown?): String? {
        if (b == null) return "ВНИМАНИЕ: об иероглифах неизвестно — движок не отдал отчёт"
        if (b.glossState == 2) {
            return "ВНИМАНИЕ: перевод иероглифов выключен — правило не собралось, " +
                "иероглифы пойдут без перевода. Это ошибка в программе, сообщите о ней."
        }
        if (b.glossState == -1) {
            return "ВНИМАНИЕ: библиотека не сообщила о переводе иероглифов — " +
                "похоже, собрана старая версия"
        }
        // Запрет включается только переключателем в библиотеке; пока он
        // выключен (banState 4), о нём говорить нечего.
        if (b.banState == 1 && b.banHits > 0) {
            return "Иероглифы: запрет перехватил ${b.banHits} раз"
        }
        if (b.glossSpans > 0) {
            return "Иероглифы: мест в ответе — ${b.glossSpans}" +
                if (b.glossState == 1) ", перевод в скобках" else ""
        }
        return null
    }
    ...
    private fun metricsReport(
        ...
        hieroglyphLine(breakdown)?.let { lines += it }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Ход с пустым вопросом провело тело агента (написал первым), и экран
        // о нём иначе не узнает. Ждётся конец хода, а не запись на диск: запись
        // может кончиться раньше, чем ход отпустит замок, и тогда перерисовка
        // не случилась бы (см. ConversationTurns.closedEvents).
        //
        // Свои ходы экран перерисовывает сам, закрыв их, — здесь их не трогать:
        // к ленте он дописывает строки, которых в ней нет. И не посреди идущего
        // хода: в поле дописываются токены, и перерисовка стёрла бы их. Если
        // ход экрана успел начаться, пропуск безвреден: закрыв свой ход, экран
        // перерисует ленту целиком, вместе с ходом агента.
        lifecycleScope.launch {
            turns.closedEvents.collect { index ->
                val byAgent = journal.history().getOrNull(index)?.question?.isEmpty() == true
        ...
        binding.buttonGenerate.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                // дошло до собеседника, а отсюда реплика ещё может не уйти:
                // ниже стоят два отказа по размеру, и оба кончаются диалогом.
                //
                // Прежний довод «после отбора» при этом не потерян, а усилен:
                // новое место стоит ещё ниже, значит свежая запись по-прежнему
                ...
                // уже на руках.
                //
                // РЕПЛИКА ПЕРЕДАЁТСЯ ВТОРЫМ ДОВОДОМ, и это не украшение. Спор
                // двух старых записей между собой редок; обычное дело — когда
                // человек говорит сейчас то, что расходится с лежащим. Сверка
                // одних записей такую пару не видит вовсе и молчит ровно там,
                // где нужнее всего.
                //
                // Передаётся именно userText, а не строка ленты: правило
                ...
                // Знаки записей считаются по САМОМУ блоку записей, той же
                // склейкой, какой его собирает журнал. Прежний счёт — длина
                // реплики минус длина вопроса — относил к записям ещё и
                // пометку об ответе без опоры, и блок сверки, и пустые строки
                ...
                suspend fun afterSent() {
                    ...
                    // размера, то есть реплика ушла в движок.
                    //
                    // ВЫШЕ ветки нулевого ответа НАМЕРЕННО. Ноль токенов
                    // означает, что собеседник не ответил, а не что человек
                    // промолчал: запрос обсчитан, реплика услышана, и в
                    // памяти ей место. Лента в этом случае не растёт — там
                    // хранится ход разговора, а он не состоялся. Это два
                    // разных предмета, и граница у них проходит в разных
                    // местах.
                    //
                    // ПОСЛЕ ОТБОРА, как и прежде: свежая запись не находится
                    // отбором на своём же ходе, и утверждение не уезжает в
                    // модель дважды — репликой и цитатой рядом с ней.
                    //
                    // ЧЕГО ЭТО МЕСТО НЕ РАЗЛИЧАЕТ: обсчитанный запрос от
                    // запроса, на котором движок упал на входе. Сюда приходят
                    // оба, и оба записываются. Промах тут в сторону лишнего, и
                    // он ограничен отказом движка — случаем редким и видимым
                    // на экране; отличить их нечем, потому что метрики движок
                    // присылает не всегда.
                    //
                    // ЧЕГО НЕ ПОКРЫВАЕТ: реплики других путей — цикл задачи и
                    // вопрос, поданный агенту во время цикла, — сюда не
                    // заходят и в память сами не ложатся.
                    //
                    // ЧЕГО НЕ ПЕРЕЖИВАЕТ: отмену корутины (уход активности).
                    // Тогда не запишется ни память, ни лента — оба конца хода
                    // теряются вместе, и это лучше, чем порознь.
                    //
                    // Разбора, стоит ли это помнить, здесь по-прежнему нет:
