...
class MainActivity : AppCompatActivity() {
    ...
    private suspend fun renderPendingReview(pending: List<Sticker>): CharSequence {
        ...
        // Подписи к трём полям строки записи, в том же порядке, в каком поля
        // идут ниже. Строчные названия полей — не небрежность: они повторяют
        // то, как поле подписано в самой строке.
        out.append("\n\nтег — пул, с которым сверяли: только горячие записи ")
        out.append("того же тега. время — когда запись создана. последняя ")
        out.append("строка — с кем запись спорит СЕЙЧАС; прячется всегда ")
        out.append("пришедшая позже, потому что проверка стоит на входе.")
