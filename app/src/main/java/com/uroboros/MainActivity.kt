...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
         ...
         *
         * ЧИСЛО ВРЕМЕННО ПОДНЯТО С 512 ДО 2000 РАДИ ОДНОГО ЗАМЕРА и подлежит
         * возврату сразу после него. Замеряется пятиминутный потолок
         * непрерывного обращения в стороже: при наблюдавшихся 163 мс на токен
         * ответ в 512 токенов укладывается примерно в 83 секунды, то есть
         * упереться в потолок штатным вопросом нельзя в принципе, и ветка
         * обрыва не срабатывала ни разу.
         *
         * ПОЧЕМУ ПРАВИТСЯ ЭТО ЧИСЛО, А НЕ ПОРОГ СТОРОЖА. Порог судит по своему
         * настоящему значению; здесь поднимается только разрешённая длина
         * ответа, то есть создаётся нагрузка, на которой предохранитель
         * срабатывает сам. Подстройка самого предохранителя ради того, чтобы
         * он сработал под наблюдателем, — другой род действия, и он в этом
         * проекте уже отвергался.
         *
         * ЧЕГО ЭТО ЧИСЛО НЕ ДЕЛАЕТ: оно не заставляет модель говорить долго,
         * а только разрешает. Короткий вопрос кончится за секунды и при
         * потолке в 2000. Длинную выдачу обеспечивает задание, а не эта
         * константа.
         *
         * ЦЕНА, ИЗ-ЗА КОТОРОЙ ЧИСЛО НЕ ОСТАВЛЯЮТ ПОДНЯТЫМ. Оно не только
         * потолок генерации: от него же считается бюджет ленты (см. оба
         * вызова `journal` перед отправкой). При 2000 у разговора отнимается
         * почти полторы тысячи токенов из общего окна — лента кончается
         * заметно раньше, и длинные вопросы перестают приниматься.
         */
        private const val ANSWER_TOKEN_LIMIT = 2000
