...
class MainActivity : AppCompatActivity() {
    ...
    /**
     * Строка о запрете иероглифов в выдаче.
     *
     * Что запрещается и чего запрет не умеет — у `build_script_ban` в
     * `gguf_lib.cpp`; здесь только перевод состояния в слова. Кричим, когда
     * запрета нет там, где он должен быть: выключенный запрет внешне
     * неотличим от включённого, пока модель не напишет иероглиф.
     *
     * «Перехвачено» — сколько раз модель тянулась к иероглифу первым выбором
     * и получила отказ. Ноль при включённом запрете — «не тянулась», а не
     * «не работает».
     */
    private fun scriptBanLine(b: GGMLEngine.DecodeBreakdown?): String {
        if (b == null) return "ВНИМАНИЕ: о запрете иероглифов неизвестно — движок не отдал отчёт"
        return when (b.banState) {
            1 -> {
                val tokens = b.banTokens
                if (b.banHits == 0L) {
                    "Иероглифы: запрет на $tokens токенов, модель к ним не тянулась"
                } else {
                    "Иероглифы: запрет на $tokens токенов, перехвачено ${b.banHits} раз — " +
                        "модель хотела написать иероглиф, и ей не дали"
                }
            }
            2 -> "Иероглифы: в словаре этой модели их нет, запрещать нечего"
            3 -> "ВНИМАНИЕ: запрет иероглифов ВЫКЛЮЧЕН — он задел бы русский текст. " +
                "Это ошибка в программе, сообщите о ней."
            0 -> "ВНИМАНИЕ: запрет иероглифов не собран — иероглифы в ответе возможны"
            else -> "ВНИМАНИЕ: библиотека не сообщила о запрете иероглифов — " +
