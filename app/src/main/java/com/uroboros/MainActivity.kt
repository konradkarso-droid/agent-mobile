...
class MainActivity : AppCompatActivity() {
     ...
     *
     * Здесь СОЗНАТЕЛЬНО нет ни заголовка, ни пояснения — только записи и
     * вопрос. Прежний заголовок "Контекст из памяти:" модель читала как тему
     * разговора и рассуждала о своей памяти вообще ("в моей памяти нет
     * информации") вместо того, чтобы пользоваться тем, что ей дали. Замена
     * его на другой заголовок лечила бы симптом; убрать проще и дешевле.
     *
     * Пояснение словами ("это записи, а не твои реплики… если ответа нет,
     * скажи прямо") было написано и снято до первого прогона: три предложения
     * стоят около полусотни токенов В КАЖДОМ запросе, а работу, ради которой
     * они писались, делает грамматика самой строки записи — см.
     * [provenancePhrase]. Проверяем сначала голую форму: если её хватит,
     * выигрыш бесплатный, если нет — будет точно известно, чего не хватило,
     * и дописывается это одной строкой сюда. Ставить всё разом нельзя: тогда
     * при успехе неизвестно, что именно сработало, и платить придётся за всё
     * сразу и навсегда.
     *
     ...
     * кавычки, которыми в языке и отделяют чужую речь от своей.
     */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonGenerate.setOnClickListener {
            ...
            lifecycleScope.launch {
                ...
                // поднял из кэша.
                //
                // Рамка считается ОТДЕЛЬНО от записей. Сейчас она крошечная —
                // только строка "Вопрос:" с переводами строк, — и разделение
                // выглядит избыточным. Оно стоит здесь именно поэтому: если
                // мы когда-нибудь допишем в запрос пояснение словами (снято
                // до первого прогона, см. buildPrompt), число вырастет на
                // сотни знаков, и рост будет виден сразу, а не растворится
                // внутри "памяти".
                //
                // Числа до 27.08.2026 с новыми напрямую не сравнивать: там
                // "память" включала старую рамку целиком.
