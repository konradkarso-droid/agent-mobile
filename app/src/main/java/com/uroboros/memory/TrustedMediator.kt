...
class TrustedMediator(context: Context) {
    ...
    /**
     * Пояснение к показаниям прибора очереди — отдельно от самих показаний.
     *
     * Разведено не здесь, а в [ReviewWitness]; здесь только проброс. Смысл
     * разведения в том, что показывающий слой прячет пояснение, не трогая
     * числа: спрятать числа этого прибора нельзя, их сверяет человек с
     * соседним снимком памяти.
     *
     * Не suspend: с диска ничего не поднимает, текст один и тот же всегда.
     */
    fun reviewWitnessNote(): String = reviewWitness.readingNote()
     ...
     *
     * НЕОБРАТИМО, и обратного действия здесь нет намеренно. Отдельный путь
     * "вернуть", который сам решал бы, кого отправить обратно в карантин, был
     * бы вторым местом, где определяется спорное, — а определяться оно должно
     * в одном.
     *
     * Это НЕ запрет поднимать бит вообще. Рядом стоит [hideForReview]: он
     * ничего не проверяет и не выбирает, а исполняет решение человека о
     * названной записи. Довод выше бьёт по механизму, выбирающему сторону, а
     * не по действию, которым выбор исполняется. Отменить приём через него
     * нельзя — там скрывают вторую сторону спора, а не принятую запись.
     *
    ...
    /**
     * Убрать в очередь запись, лежащую в памяти. Исполнение решения человека,
     * а не проверка.
     *
     * Проброс без логики — зачем метод нужен, кто вправе его звать и чего он
     * не умеет, сказано в KDoc HourglassMemory.hideForReview; повторять здесь
     * незачем, две копии разойдутся.
     *
     * Что добавляет фасад: путь сюда только от человека, как и у приёма.
     * Агенту его не давать — иначе он получит и то и другое, то есть полную
     * власть над тем, что прочитает потом как факт.
     *
     * @return прошёл ли запрос. Ложь означает, что запись осталась в выдаче.
     */
    suspend fun hideForReview(id: Long): Boolean = hourglass.hideForReview(id)

    /**
     * Отвергнуть запись. Проброс без логики: что это значит, почему необратимо
     * с экрана и зачем путь — в KDoc HourglassMemory.reject и RejectPath. Путь
     * только от человека, как у приёма и скрытия.
     */
    suspend fun reject(id: Long, via: RejectPath): RejectOutcome = hourglass.reject(id, via)

    /**
     * Запись по номеру — в любом состоянии: видимая, в очереди, отвергнутая.
     *
     * Это НЕ обращение к памяти: ни счёт обращений, ни прогрев слоя, ни польза
     * не трогаются. Человек смотрит на одну запись, чтобы решить её судьбу, и
     * засчитывать это как обращение значило бы греть запись, которую как раз
     * собираются отвергнуть.
     *
     * Зачем вход по номеру вообще. Экран «Показать» выдаёт два десятка записей
     * по рангу или по словам, и до части записей он не дотягивается никаким
     * запросом: длинная запись не проходит отбор по словам вовсе. Номер
     * дотягивается до любой.
     *
     * null — записи с таким номером нет. Сбой базы не глотается: он приходит
     * исключением, чтобы «нет такой» и «память не ответила» не выглядели
     * одинаково.
     */
    suspend fun getRecord(id: Long): Sticker? = dao.getById(id)
