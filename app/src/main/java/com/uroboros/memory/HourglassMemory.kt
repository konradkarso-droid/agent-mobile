...
class HourglassMemory(private val dao: StickerDao) {
    ...
    /** Запись-кандидат вместе с обеими мерами, по которым решается её судьба. */
     ...
     *
     * Известное и НЕ тронутое в этой правке: dao.search(word, limit) ограничен
     * по каждому слову отдельно, поэтому очень частое слово теоретически может
     * вытеснить из выборки годную запись ещё до всякого взвешивания. На базе в
     * 13 записей это недостижимо; лечить вслепую нечего.
     */
    private suspend fun searchByWords(
        ...
        // Собираем кандидатов и запоминаем, КАКИЕ слова нашли каждого из них.
        // Множество, а не счётчик: одно и то же слово, встретившееся в записи
        // трижды, не должно считаться за три совпадения.
        val candidates = LinkedHashMap<Long, Sticker>()
        val hits = HashMap<Long, MutableSet<String>>()

        for (word in words) {
            for (sticker in dao.search(word, limit)) {
                if (sticker.layer == Layer.RED.name) continue
                if (sticker.layer !in allowedLayers) continue
                candidates[sticker.id] = sticker
                hits.getOrPut(sticker.id) { mutableSetOf() }.add(word)
            }
        }
        ...
        for (sticker in candidates.values) {
            ...
            val matchedChars = matched.sumOf { it.length }
