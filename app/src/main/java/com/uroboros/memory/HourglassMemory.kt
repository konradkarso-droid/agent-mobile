...
// Два правила ниже — что считать значимым словом — читают поиск по вопросу
// и ночной сон (memory.dream.DreamWeaver). Лежат на верхнем уровне, чтобы у
// обоих было одно определение: слово, служебное для поиска, служебное и для
// сна, и расходиться этим спискам не из-за чего.

/** Короче этого слово совпадёт с чем угодно. */
internal const val MIN_WORD_LENGTH = 4
...
class HourglassMemory(
     ...
     *    которые по часам уже остыли, числятся горячими. Поэтому его зовёт
     *    каждый, кто делит записи по слою: выборка, проверка при сохранении,
     *    показ спора и ночной судья. Новый такой читатель обязан звать его тоже;
     ...
     *  - записи об идентичности: classify вернул RED с null-интервалом, значит
     *    стоит явная метка (Prism.IDENTITY_TAG), и трогать их нельзя.
     *
    ...
    suspend fun saveEventChecked(sticker: Sticker): SaveOutcome {
        ...
        //
        // Уборка идёт первой и внутри того же try: горячий пул определяется
        // слоем, а слой без уборки отстаёт от часов (см. migrateExpired). Пул,
        // снятый без неё, — не тот пул, по которому правило должно судить, так
        // что её сбой считается несостоявшимся пулом и виден в приборе очереди
        // как сбой, а не проглатывается.
        val hotPool = try {
            migrateExpired()
    ...
    /**
     * Отвергнуть запись по решению человека: он счёл её неверной.
     *
     * ЗАЧЕМ ОТДЕЛЬНОЕ СОСТОЯНИЕ. Скрыть запись можно было и раньше — битом
     * очереди. Но очередь значит «ждёт решения», и запись, которую человек уже
     * счёл неверной, оставалась в ней навсегда: принимать её нельзя, а другого
     * выхода из очереди нет. Очередь с настоящим спором поэтому не пустела
     * никогда, а разбор шёл по кругу: принял одну сторону — убрал другую — в
     * очереди столько же. Отвергнутая запись скрыта тем же битом, но из очереди
     * уходит и считается отдельно.
     *
     * ПОЧЕМУ ТЕМ ЖЕ БИТОМ. Все пути, которые не должны видеть скрытое, — выдача,
     * горячий пул проверки, судья, сон — уже держатся на нём. Отдельный флаг
     * пришлось бы добавить в каждый, и забытый путь протекал бы отвергнутым
     * молча. Здесь забыть можно только в обратную сторону: место, читающее
     * очередь, но не знающее об отвержении, покажет отвергнутую как ждущую —
     * это видно на экране, а в ответы она не попадёт.
     *
    ...
    suspend fun getRejected(): List<Sticker> = dao.getRejected()
    ...
    suspend fun disputesOf(sticker: Sticker): DisputeReport {
        return try {
            // Деление на горячих и остывших противников — это деление по слою,
            // поэтому слои сперва доводятся до часов. Без этого противник,
            // остывший по времени, показывался бы горячим, и исход «спор распался
            // сам» не наступал бы, пока кто-нибудь не задаст вопрос.
            migrateExpired()
            var comparisons = 0
