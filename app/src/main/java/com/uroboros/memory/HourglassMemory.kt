...
class HourglassMemory(
     ...
     *
     * ПАДЕНИЕ СРАВНЕНИЯ ПОДНИМАЕТ БИТ. Подстановка в благополучную сторону
     * здесь недопустима: второй попытки не бывает, [RiskTrigger.evaluate]
     * зовётся один раз за жизнь записи, так что непроверенное ушло бы в
     * горячую память навсегда и молча. Промах в эту сторону невосстановим;
     * промах в обратную стоит одной лишней записи в очереди, которую человек
     * снимет.
     *
     ...
     *  - отсюда следствие: систематическая поломка сравнения выглядит в
     *    очереди как всплеск споров — очередь растёт, человек её разбирает, а
     *    механизм мёртв. Отличить эти два случая можно только счётом входов
     *    (см. [ReviewWitness]); в самой очереди следа сбоя нет, и в логе тоже —
    ...
    suspend fun saveEventChecked(sticker: Sticker): SaveOutcome {
        ...
        sticker.reviewPending = sticker.reviewPending || entry != ReviewEntry.CLEAN
