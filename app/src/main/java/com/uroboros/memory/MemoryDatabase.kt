...
abstract class MemoryDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // Спрошенный сон и ответы о нём (см. dream.Dream.askedAt). У прежних
        // снов null и ноль — правда: спрашивать до этой версии было нечем.
        val MIGRATION_20_21 = object : Migration(20, 21) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE dreams ADD COLUMN askedAt INTEGER")
                db.execSQL("ALTER TABLE dreams ADD COLUMN answeredCount INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE dreams ADD COLUMN lastAnsweredAt INTEGER")
            }
        }

        // Касания без подсказки агента (см. Sticker.userMatchUnpromptedCount).
        // Ноль у прежних записей — правда: до этой версии подсказанное касание
        // от неподсказанного не отличали, и засчитать задним числом нечего.
        val MIGRATION_21_22 = object : Migration(21, 22) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE stickers ADD COLUMN userMatchUnpromptedCount INTEGER NOT NULL DEFAULT 0")
            }
        }
        ...
        // Итог шага «строка о себе» в строку ночи (DreamNight.selfLineOutcome).
        // Прежним ночам — NULL, и это правда: шага тогда не было. Ночи без
        // кнопки тоже остаются с NULL — самостоятельный сон строку не предлагает.
        val MIGRATION_23_24 = object : Migration(23, 24) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE nights ADD COLUMN selfLineOutcome TEXT")
            }
        }
