...
object RiskTrigger {
     ...
     *
     * Одно исключение. Когда связкой разложены ОБЕ фразы, спорить могут
     * несколько пар частей, и отрицание в них может стоять с разных сторон
     * («не рубанок, а стамеска» против «ножовка, а не рубанок»). Признак один
     * на вид, слова в нём — объединение по всем спорящим парам частей, и у
     * [NEGATION] тогда названы обе стороны. Взять одну пару частей вместо
     * объединения нельзя: какую — зависело бы от порядка аргументов, и
     * признаки перестали бы переворачиваться вместе с текстами.
     *
     ...
     * затем отрицание у одной стороны и расхождение чисел.
     */
     ...
     *  - «X не Y, а Z» → «X не Y» и «X Z»;
     *  - «X — Z, а не Y» → «X — Z» и «X — не Y».
     *
     * ЧЕГО НЕ УМЕЕТ:
     ...
     *  - ЛОЖНЫЕ СПОРЫ ПРАВИЛА ПЕРЕХОДЯТ НА ЧАСТИ. Отрицание засчитывается,
     *    даже когда одна сторона отрицает одно, а другая утверждает другое.
     *    Поэтому «не рубанок, а стамеска» против «не рубанок» — спор: часть
     *    «…— стамеска» против «…— не рубанок». Разложение этот класс не
     *    создаёт, у целых фраз он тот же («не рубанок с деревянной колодкой»
     *    против «рубанок с железной»), но случаев с ним становится больше.
     *
