 ...
 * позднее), время последней реплики владельца — в памяти процесса
 * ([ownerReplied]). После комы второе неизвестно, и вопрос, если он хоть раз
 * был, считается ждущим ответа до первой реплики нового процесса: неизвестное
 * решается в сторону «не спрашивать». Цена — после перерыва в жизни первая
 * реплика вопроса не несёт.
 *
...
object CuriosityAsk {
     ...
     * @param lastOwnerReplyAt когда в этом процессе ушла в движок последняя
     *   реплика владельца; null — ни одной (см. «ГДЕ ЖИВЁТ» выше).
     */
    ...
    @Volatile
    private var lastReplyAt: Long? = null

    /** Реплика владельца ушла в движок в момент [at]. */
    fun ownerReplied(at: Long) {
        lastReplyAt = at
    }

    /** Когда в этом процессе ушла последняя реплика владельца; null — ни одной. */
    fun lastOwnerReplyAt(): Long? = lastReplyAt
}
