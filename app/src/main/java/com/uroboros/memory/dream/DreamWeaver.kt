...
object DreamWeaver {
     ...
     *
     * ПОРОВНУ, А НЕ ПО СТАРШИНСТВУ. Пары записей перебираются по номерам, и сны
     * копятся в том же порядке: сначала все пары самой старой записи, потом
     * следующей. Отрезать хвост значило бы отдать все места нескольким старым
     * записям. На живой памяти так и было: одна старая запись стояла краем в
     * каждом показанном сне, а почти половина записей не приснилась ни разу.
     *
     * Правило: следующим берётся сон, в котором есть запись, снившаяся реже
     * всех (меньшее число её снов среди уже взятых), а при равенстве — сон,
     * самая частая запись которого снилась реже. Так запись без снов получает
     * место раньше, чем кто-то получит лишний, и одна запись не собирает все
     * места, даже будучи краем каждой пары. При полном равенстве — прежний
     ...
     *    каждый из них приносит запись, которой ещё не снилось: это уже не
    ...
    internal fun fairShare(dreams: List<Woven>, limit: Int): List<Woven> {
        ...
        repeat(limit) {
            ...
            taken[best] = true
            for (id in dreams[best].recordIds.distinct()) uses[id] = (uses[id] ?: 0) + 1
        }
        return dreams.filterIndexed { i, _ -> taken[i] }
    }
