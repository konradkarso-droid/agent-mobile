 ...
 * выходов (спросить, поискать, попробовать) здесь нет и не закладывается.
 * Выбор выхода — отдельный механизм, который будет читать это давление, а не
 * жить внутри него.
 *
 ...
 *    вес [RECALLED_WEIGHT];
 *  - результат собственного действия агента, вес [OWN_WEIGHT]. Источника
 *    пока нет — отчёты агента не снятся по построению (см. [DreamWeaver]), —
 *    поэтому слагаемое всегда ноль и так и показывается.
 ...
 *  - не знает, что будет с давлением дальше: выходов нет, пружина только
 *    копит и показывает.
 */
object CuriosityPressure {
    ...
    /** Сон, давший наибольший вклад, и сам вклад. */
    data class Leader(val brief: Brief, val contribution: Int)
    ...
    fun measure(dreams: List<Dream>, byId: (Long) -> Sticker?, now: Long): Result {
        ...
        return Result(
            ...
            leader = leader?.let { (dream, records) ->
                Leader(Brief(dream.kind, records.map { it.content }), leaderContribution)
            },
        )
    }
     ...
     *
     * [pickedThisTurn] — сны, подхваченные репликой этого хода; стоят в первой
     * строке, чтобы владелец сверил, верно ли понято.
     */
    fun meter(result: Result, pickedThisTurn: List<Brief> = emptyList()): String = buildString {
        ...
        append(", своё ").append(result.own).append(" (×").append(OWN_WEIGHT).append(", источника пока нет)")
