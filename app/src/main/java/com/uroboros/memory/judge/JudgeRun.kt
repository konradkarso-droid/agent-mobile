 ...
 *
 * Как выбираются пары. Берутся записи из горячих слоёв — тех, что модель и так
 * читает в ответах. Остывшая запись в ответ не попадает, значит и ввести в
 * заблуждение никого не может, и спор с ней ничего не исправит. Метка записи
 * НЕ учитывается, в отличие от правила: метку сегодня никто не проставляет, и
 * фильтр по ней молча прятал бы споры между темами, когда её начнут ставить.
 *
