...
interface StickerDao {
     ...
     *
     * РЕПЛИКИ ЛЕНТЫ — ТОЖЕ В САМОМ ЗАПРОСЕ, по той же причине. [excluded] —
     * реплики владельца, которые модель видит в этом запросе (см. параметр
     * excludedTexts у HourglassMemory.getContextWithSummary). Автозапись кладёт
     * каждую реплику, и они — самые новые записи базы. Отсекай их после
     * лимита, и получились бы две поломки: слово, частое в разговоре, набрало
     * бы limit реплик ленты и вытеснило живую запись, а лестница остановилась
     * бы на ступени, где нашлись одни реплики, и короче не пошла бы. Условие
     * ловит точный текст — автозапись кладёт реплику без обрезки; запись того
     * же текста в другой форме отсекается вторым забором в лестнице.
     *
     * Пустой список — не исключать ничего: Room разворачивает его в `NOT IN ()`,
     * а это в SQLite всегда истина. Длина списка — число ходов ленты плюс
     * один, то есть десятки строк: до потолка переменных SQLite (999 на
     * старых версиях) далеко.
     */
    @Query(
    ...
    /**
     * Отвергнуть запись: скрыть и отметить момент и путь решения одним запросом,
     * чтобы не было мгновения, когда запись отмечена, но видна, или скрыта, но
     * числится ждущей, или отмечена без пути.
     *
     * ПЕРВОЕ РЕШЕНИЕ ОСТАЁТСЯ. Уже отвергнутую запись запрос не трогает: иначе
     * второе нажатие другим путём перетёрло бы и момент, и путь первого, и из
     * базы нельзя было бы узнать, почему запись на самом деле замолчала.
     *
     * @return сколько записей изменено: 1 — отвергнута сейчас; 0 — записи с
     *         таким номером нет или она уже отвергнута. Чем именно из двух,
     *         запрос не различает — это видно по [getById].
     */
    @Query(
        "UPDATE stickers SET reviewPending = 1, rejectedAt = :at, rejectedVia = :via " +
            "WHERE id = :id AND rejectedAt IS NULL"
    )
    suspend fun reject(id: Long, at: Long, via: String): Int
     ...
     *
     * ЗАЧЕМ ОТДЕЛЬНЫЙ ЗАПРОС. До его появления бит поднимался ровно одним
     * способом — при вставке новой записи. Значит запись, однажды попавшая в
     * выдачу, оставалась там навсегда, чем бы ей ни противоречило пришедшее
     * после: проверка стоит на входе и назад не смотрит. Полная перезапись
     * строки через @Update для этого не годится — она затирает слой и
     * счётчики (см. KDoc интерфейса).
     *
     * КТО РЕШАЕТ. Никакой механизм этот запрос сам не зовёт и звать не должен:
     * правило противоречия отвечает «спорят или нет» и не знает, какая из двух
     * сторон верна. Выбор стороны — решение человека, и запрос существует
     * только чтобы это решение можно было исполнить.
     *
     * НАПРАВЛЕНИЕ ПРОМАХА. Скрытие обратимо: запись цела, попадает в очередь и
     * принимается назад одним нажатием. Этим оно отличается от приёма, который
     * необратим, и поэтому здесь допустимо то, что для приёма запрещено, —
     * например применить действие сразу к нескольким записям.
     *
     * ЧЕГО НЕ УМЕЕТ. Причина поднятия здесь не сохраняется, как не сохраняется
     * и при вставке: в очереди запись, скрытая рукой человека, неотличима от
     * скрытой проверкой. Повторный вызов на уже скрытой записи ничего не
     * меняет и ошибкой не является.
     */
