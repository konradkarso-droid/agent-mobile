...
object QueryUrgencyClassifier {
    ...
    // эвристика без реальной морфологии. Список окончаний намеренно узкий (только
    // относительно однозначные формы) — цель не поймать все императивы, а не давать
    // ложных HEAVY: неуверенный случай должен уходить в AMBIGUOUS → Jaccard, а не
    // ошибочно классифицироваться как HEAVY на грамматике одной. Порог/список
    // подлежит калибровке позже через consolidation pass (item 6), как и gray-zone
    // в item 8a.
    private val IMPERATIVE_SUFFIXES = listOf("айте", "яйте", "уйте", "ите", "ьте", "ай", "яй", "уй")
