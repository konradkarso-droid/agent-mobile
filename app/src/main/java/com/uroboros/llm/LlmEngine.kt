...
class LlmEngine(
    ...
    private companion object {
         ...
         * вместе с повторами.
         *
         * ЧЕГО ЖДАТЬ, ЧЕСТНО. 1.1 может не хватить. Штраф работает по
         * отдельным токенам, а здесь повторяется целая фраза, и держит её
         * пример в ленте — сила, с которой сотые доли штрафа не сравнимы.
         * Если после правки повтор останется, следующий шаг НЕ дальнейший
         * подъём (там начнётся порча языка), а окно `penaltyLastN`: сейчас
         * оно 64 токена, то есть повтор, отстоящий дальше, вообще не
         * штрафуется. Расширять его надо с той же оглядкой — чем шире окно,
         * тем больше падежных окончаний под штрафом.
         */
        const val SAMPLER_PARAMS_JSON = "{\"repeat_penalty\":1.1,\"penaltyLastN\":64}"
    }
}
