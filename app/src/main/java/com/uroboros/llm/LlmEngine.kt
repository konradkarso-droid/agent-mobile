...
class LlmEngine(
     ...
     *
     * ПЕРЕОТКРЫТ 26.08.2026, но по ДРУГОЙ мишени — см. [THREADS_OVERRIDE].
     * Записанное выше остаётся верным: генерация от четвёртого потока не
     * выигрывает ничего. Проверяется обратная сторона той же правки —
     * что станет с ОБСЧЁТОМ, когда потоков у него станет четыре вместо
     * восьми. В прошлый раз это осталось неизмеренным: обсчитывать было
     * нечего, стена приходила из кэша.
     */
    ...
    private companion object {
        ...
        /**
         * Число потоков, задаваемое движку напрямую.
         *
         * ЭКСПЕРИМЕНТ, ПОСТАВЛЕН 26.08.2026. Проверяется догадка: не мешают ли
         * обсчёту запроса слабые ядра процессора.
         *
         * Что мерили. Обсчёт запроса идёт около 10 токенов в секунду при
         * генерации 5.4 — то есть быстрее всего в 1.8 раза, тогда как обсчёт
         * читает веса ОДИН РАЗ на всю пачку, а генерация — ради каждого
         * токена, и разница должна быть в десятки раз. Первый подозреваемый
         * (сжатие KV-кэша) проверен и снят, см. [KV_CACHE_TYPE].
         *
         * Почему подозрение падает на потоки. В логе загрузки стоит
         * `threads_batch=8`. Snapdragon 865 — это одно сильное ядро, три
         * средних и ЧЕТЫРЕ СЛАБЫХ. Пачка считается всеми потоками
         * одновременно, и все ждут самый медленный. Четыре слабых ядра в такой
         * упряжке способны тормозить всю работу, и тогда обсчёт ЧЕТЫРЬМЯ
         * потоками окажется быстрее, чем восемью.
         *
         * Оговорка честная: это же число задаёт и потоки генерации, отдельного
         * параметра для обсчёта библиотека наружу не выводит (поиск по
         * `threadsBatch` в репозитории пуст). Значит генерация заодно уйдёт с
         * трёх потоков на четыре — про что уже известно, что это ноль
         * (см. [applyThreadMode]). Терять на этом нечего.
         *
         * Число потоков НЕ входит в отпечаток папки кэша (см. [loadIdentity]),
         * поэтому стена не пересобирается и первый же запуск даёт годный
         * замер.
         *
         * ОТКАТ — вернуть `params.threads` в обоих вызовах загрузки, и тогда
         * число потоков снова назначает профиль [THREAD_MODE_PERFORMANCE].
         *
         * Если обсчёт не ускорится — записать отрицательный результат сюда же
         * и откатить. Причина тогда лежит внутри нативной части библиотеки,
         * куда наружу ходу нет, и это тупик того же сорта, что угадывание
         * токенов.
         */
