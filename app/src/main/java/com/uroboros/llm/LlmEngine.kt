...
class LlmEngine(
    ...
    private companion object {
        ...
        /**
         * Число потоков, запрашиваемое у движка при загрузке.
         *
         * ЭКСПЕРИМЕНТ. Откат — одно число здесь, обратно на `params.threads`
         * в обоих путях загрузки.
         *
         * Почему раньше этот параметр был под запретом. Он задаёт ОДНО число
         * сразу двум разным работам: генерации (токен за токеном, узкое место —
         * память) и обсчёту запроса (пачкой, узкое место — вычислитель).
         * Профиль потоков mode=2 разводил их сам: генерации 3 потока, обсчёту 8.
         * Задать здесь 4 значило поднять генерацию с 3 до 4, но уронить обсчёт
         * с 8 до 4 — размен заведомо невыгодный, пока обсчёт стоит полторы
         * минуты на каждом холодном запуске.
         *
         * Что изменилось. Дисковый кэш системной стены снял с обсчёта основную
         * работу: стена поднимается с диска за 0.0 с, обсчитывать остаётся
         * только текст самого вопроса — десятки токенов вместо тысячи с лишним.
         * Цена падения обсчёта стала мала, а генерация — по-прежнему всё время
         * ответа целиком.
         *
         * Чего ждём: до ~30% по величине «мс на токен» (база 175-183), если
         * планировщик не разбросает четвёртый поток на медленные ядра.
         * Основание — измеренное на этом телефоне: потоков в 1.5 раза больше
         * дало 1.38 скорости, то есть упора в пропускную способность памяти
         * нет и добавка ещё не бессмысленна.
         *
         * Чего НЕ ждём и на что смотреть первым: заданное число и работающее —
         * разные величины. Профиль потоков mode=2 задаётся ПЕРЕД загрузкой и
         * вполне может оказаться сильнее этого параметра. Тогда правка не
         * сделает ничего, и видно это будет не по секундомеру, а сразу — по
         * строке `threads_gen=` в собственном логе движка, ещё до первого
         * вопроса.
         *
         * Риск, принятый сознательно: на прогонах, где кэш промахивается
         * (сменилась модель, сменилась стена), полная цена обсчёта вернётся —
         * и вернётся уже на четырёх потоках вместо восьми, то есть будет вдвое
         * дороже прежнего. Терпимо: такой прогон бывает раз на смену модели.
         */
