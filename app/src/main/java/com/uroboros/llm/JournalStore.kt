...
class JournalStore(context: Context) {
     ...
     * поэтому собери это из `count()` и `clear()` — заход остановился бы
     * навсегда на втором захвате. Причина подробнее в KDoc самого [gate].
     *
     ...
     *
     * ОТКАЗ ЧТЕНИЯ — `null`, А НЕ НУЛИ. Отличие от [count], и оно
     * намеренное: показать «в архиве 0» сразу после закрытия разговора
     * значило бы объявить потерю обычной работой. Пусть человек увидит «не
     * читается».
     */
